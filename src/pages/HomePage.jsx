...
const HomePage = ({ data, schedule, grades, attendance }) => {
  ...
  const filtered = schedule?.filter((item) => item.date === dateToday);
  const filteredGrades = grades?.filter((item) => item.date === dateToday);
  ...
  const todayMissed = filteredAttendance?.filter(
        ...
        <div className="dashboard">
          ...
          <div className="card">
            ...
            <div className="attendance-info">
              <p>
                ❌ Всего пропусков: <b>{missedCount || 0}</b>
              </p>
              <p>
                📅 Сегодня пропусков: <b>{todayMissed || 0}</b>
              </p>
            </div>
